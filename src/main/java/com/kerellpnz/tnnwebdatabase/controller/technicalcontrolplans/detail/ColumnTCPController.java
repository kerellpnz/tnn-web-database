package com.kerellpnz.tnnwebdatabase.controller.technicalcontrolplans.detail;

import com.kerellpnz.tnnwebdatabase.controller.technicalcontrolplans.BaseTCPController;
import com.kerellpnz.tnnwebdatabase.dao.BaseEntityDAO;
import com.kerellpnz.tnnwebdatabase.entity.technicalcontrolplans.detail.ColumnTCP;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.servlet.ModelAndView;

import java.util.List;

@Controller
@RequestMapping("/tcp/ColumnTCPs")
public class ColumnTCPController extends BaseTCPController<ColumnTCP> {

    @Autowired
    private BaseEntityDAO<ColumnTCP> TCPDAO;

    @GetMapping("/showAll")
    public ModelAndView showTCPs(@RequestParam(name="operation", required = false) String operation) {
        ModelAndView mv = new ModelAndView("index");
        mv.addObject("title", "ПТК-Стойка");
        mv.addObject("tcpClass", "ColumnTCPs");
        mv.addObject("userClickTCP", true);
        if(operation!=null) {
            if(operation.equals("tcp")) {
                mv.addObject("message", "Пункт успешно добавлен!");
            }
        }
        return mv;
    }

    @PostMapping("/save")
    public String saveTCP(@ModelAttribute("tcp") ColumnTCP tcp) {
        TCPDAO.saveOrUpdate(tcp);
        return "redirect:/tcp/ColumnTCPs/showAll?operation=tcp";
    }

    @GetMapping("/delete/{id}")
    public String deleteTCP(@PathVariable int id) {
        TCPDAO.delete(ColumnTCP.class, id);
        return "redirect:/tcp/ColumnTCPs/showAll";
    }

    @GetMapping("json/getAll")
    @ResponseBody
    public List<ColumnTCP> getTCPs() {
        return TCPDAO.getAll(ColumnTCP.class);
    }

    @ModelAttribute("tcp")
    public ColumnTCP getTCPModel() {
        return new ColumnTCP();
    }
}
