...
public class CaseBottomController extends BaseUnitController {
    ...
    public ModelAndView showAll(@RequestParam(name="parameter", required = false) String parameter) {
        ...
        if(parameter != null) {
            switch (parameter) {
                ...
                case "error" -> mv.addObject("message", "Номер Днища не является числом!");
                case "duplicate" -> mv.addObject("message", "ВНИМАНИЕ: Днище с такими характеристиками уже существует!");
            }
        }
        return mv;
    }
    ...
    public ModelAndView showFormForAdd() {
        ModelAndView mv = modelInitializeEdit("Новое днище");
    ...
    private ModelAndView showFormForUpdate(CaseBottom entity) {
        ModelAndView mv = modelInitializeEdit("Характеристики днища");
        ...
        for(CaseBottom tempEntity : entities) {
            if (entity.equals(tempEntity)) {
                if (entity.getId() != tempEntity.getId())
                    return sendMessage("ОШИБКА: Днище с таким характеристиками уже существует!", model);
            }
        }
    ...
    private void modelInitialize(Model model) {
        ...
        model.addAttribute("title", "Характеристики днища");
